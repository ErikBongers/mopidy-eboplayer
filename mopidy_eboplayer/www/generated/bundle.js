...
function createModelEvent(event, detail) {
	return new CustomEvent(event, {
		detail,
		bubbles: true,
		composed: true,
...
var EboModelEventTargetClass = class extends EventTarget {
...
var Model = class extends EboModelEventTargetClass {
	currentTrack = null;
	selectedTrack = null;
	volume = 0;
...
//#endregion
//#region mopidy_eboplayer/www/typescript/controllers/browseController.ts
var BrowseController = class {
	...
	async diveIntoBrowseResult(label, uri, type, addTextFilterBreadcrumb) {
		if (type == "track") {
			let track = await this.controller.getExpandedTrackModel(uri);
			if (track.album?.albumInfo?.uri) this.controller.viewController.showAlbum(track.album?.albumInfo?.uri, uri);
			return;
		}
		if (type == "album") this.controller.viewController.gotoAlbum(uri);
		if (type == "radio") this.controller.getExpandedTrackModel(uri).then(() => {
			this.controller.viewController.showRadio(uri);
		});
	...
	async setFavoritesFilter() {
		...
		let favoritesRef = (await this.controller.cache.getAllRefsCached()).playlists.find((res) => res.item.name == favoritesName);
	...
	async fetchRefsForCurrentBreadCrumbs() {
		...
		if (lastCrumb instanceof BreadCrumbRef) {
			if (lastCrumb.data.type == "playlist") {
				...
				let results$1 = await Refs.transformRefsToSearchResults(this.controller.cache, playlistItems);
...
var PlayerBarView = class extends ComponentView {
	...
	showHideInfo() {
		...
		if (currentView != "#NowPlaying") show_info = true;
...
var EboAlbumTracksComp = class EboAlbumTracksComp extends EboComponent {
	...
	async render(shadow) {
		...
		if (this.albumInfo) (await this.albumInfo.getTrackModels()).forEach((track) => {
			...
			tdHeart.querySelector("ebo-button.heartButton").on("pressedChange.eboplayer", (ev) => {
...
var EboListButtonBar = class EboListButtonBar extends EboComponent {
	...
	render(shadow) {
		...
		this.shadow.getElementById("btnDisplayMode").on("pressedChange.eboplayer", (ev) => {
...
var EboBigAlbumComp = class EboBigAlbumComp extends EboComponent {
	...
	render(shadow) {
		...
		shadow.getElementById("btnFavorite").on("pressedChange.eboplayer", (ev) => {
...
var RememberedView = class extends ComponentView {
	...
	bind() {
		this.state.getModel().on("remembersChanged.eboplayer", async () => {
			this.component.rememberedList = await this.state.getController().cache.getRemembersCached();
		});
		this.component.on("deleteRemember.eboplayer", async (ev) => {
...
var EboGenresComp = class EboGenresComp extends EboComponent {
	...
	static observedAttributes = ["album_title", "album_genres"];
	_genreDefs = [];
	get genreDefs() {
	...
	update(shadow) {
		...
		if (this.genreDefs.length != 0) {
			...
			while (nextIndex < this.genreDefs.length && this.genreDefs[nextIndex].genreDef.level == 0) nextIndex = this.renderGenreDef(container, nextIndex, -1, albumGenres);
		}
		this.getActiveAncestors(shadow).forEach((ancestor) => {
	...
	renderGenreDef(container, index, parentLevel, albumGenres) {
		...
		if (hasChildren) {
			...
			while (nextIndex < this.genreDefs.length && this.genreDefs[nextIndex].genreDef.level == genreDef.genreDef.level + 1) nextIndex = this.renderGenreDef(newContainer, nextIndex, genreDef.genreDef.level, albumGenres);
			return nextIndex;
		}
	...
	setSummary(element, name, albumGenres) {
		element.textContent = name;
		if (albumGenres.includes(name)) element.classList.add("albumSelection");
	}
	showLevel(level) {
	...
	updatePlaceholders(shadow) {
		...
		el = shadow.getElementById("genres");
		if (el != null) {
			value = this.getAttribute("album_genres") ?? "";
			updater = EboGenresComp.updaters["album_genres"];
			if (updater != null) value = updater(value, shadow, el);
			node = el.childNodes.item(0);
			if (node == null) el.textContent = ["", ""].join(value);
			else node.nodeValue = ["", ""].join(value);
...
//#endregion
//#region mopidy_eboplayer/www/typescript/components/eboTopBar.ts
var EboTopBar = class EboTopBar extends EboComponent {
	get message() {
	...
	update(shadow) {
		...
		switch (this.page) {
			...
			case "#Genres":
				...
				nowPlayingBtn.style.display = "block";
				break;
			default: return unreachable(this.page);
		}
		this.updateMessage();
	}
	updateMessage() {
		let message = this.message;
		if (this.tmpMessage.type != MessageType.None) message = this.tmpMessage;
		let contentHeadline = this.getShadow().getElementById("contentHeadline");
		if (message.message) contentHeadline.innerText = message.message;
		else contentHeadline.innerText = "";
		switch (message.type) {
...
//#endregion
//#region mopidy_eboplayer/www/typescript/views/topBarView.ts
var TopBarView = class extends ComponentView {
	constructor(state, component) {
		super(state, component);
	}
	bind() {
