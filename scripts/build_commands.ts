...
function main() {
    ...
    outFile = fs.createWriteStream(outputFileName); //todo: this is a global. Make it a dependency?
...
function writeModule(module: Module) {
    ...
    writeLine(`${exportWord}class ${moduleClassName} {`, 0);
    writeLine(`${protectedWord}mopidy: Mopidy;`, 4);

    for(let childModule of module.children) {
        writeLine(`${childModule.name}: ${getModuleClassName(childModule)};`, 4);
    }

    writeLine("", 0);
    writeLine(`constructor(mopidy: Mopidy) {`, 4);
    writeLine(`this.mopidy = mopidy;`, 8);
    for(let childModule of module.children) {
        writeLine(`this.${childModule.name} = new ${getModuleClassName(childModule)}(mopidy);`, 8);
    }
    writeLine("}", 4);
...
function writeFunction(funcDef: FuncDef, indent: number) {
    ...
    writeLine(`return this.mopidy.send("${funcDef.key}"${paramsObject}) as ${returnType.replaceAll(" ", "")};`, indent+4);
    writeLine("}", indent);
}
